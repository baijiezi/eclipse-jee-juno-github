...
public class JobSendMail {
	...
	public void jobSend(){
		...
		List<HashMap> ls = ConnUtil.readStringXmlOut(result.replace("&", "").replace("\23mm", "/23mm"));
		for(int a=0;a<ls.size();a++){
            ...
            de.setOutpatienTunit(ls.get(a).get("Unit").toString()); // 拿到值
            de.setSpecifiCation(ls.get(a).get("Specification").toString()); // 拿到值
