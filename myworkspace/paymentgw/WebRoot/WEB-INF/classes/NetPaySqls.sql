...
CREATE TABLE IF NOT EXISTS `ali_pay_receive_detail` (
  ...
  `total_fee` bigint(20) NOT NULL COMMENT '交易总价',
...
-- ----------------------------
-- Table structure for `ten_pay_package_receive_detail`
-- 财付通小钱包交易返回数据详细记录
-- ----------------------------
CREATE TABLE IF NOT EXISTS `ten_pay_package_receive_detail` (
  `ID` int(10) NOT NULL AUTO_INCREMENT COMMENT '数据库记录自动ID',
  `bank_type` char(8) NOT NULL COMMENT '默认银行',
  `trade_mode` char(8) NOT NULL COMMENT '交易模式',
  `discount` char(8) NOT NULL COMMENT '折扣',
  `input_charset` char(8) NOT NULL COMMENT '字符集',
  `total_fee` bigint(20) NOT NULL COMMENT '总交易额',
  `transport_fee` bigint(20) NOT NULL COMMENT '手序费',
  `notify_id` char(225) NOT NULL COMMENT '通知ID',
  `transaction_id` char(36) NOT NULL COMMENT '财付通流水号',
  `sign` char(225) NOT NULL COMMENT '加密串',
  `partner` char(32) NOT NULL COMMENT '合作商户',
  `product_fee` bigint(20) NOT NULL COMMENT '商品价格',
  `time_end` char(24) NOT NULL COMMENT '交易结束时间',
  `out_trade_no` char(24) NOT NULL COMMENT '外部订单号：多支付流水ID',
  `trade_state` char(8) NOT NULL COMMENT '交易状态',
  `sign_type` char(12) NOT NULL COMMENT '加密方式',
  `fee_type` char(8) NOT NULL COMMENT '货币类型',
  `add_time` bigint(20) NOT NULL COMMENT '数据添加时间',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8
...
CREATE TABLE IF NOT EXISTS `net_pay_data_exception` (
  ...
  `totalFee` bigint(20) NOT NULL COMMENT '交易总金额:分',
  `payType` int(1) NOT NULL COMMENT '多支付实际交易类型',
  `payStatus` int(1) NOT NULL DEFAULT '12' COMMENT '交易状态 默认：已成功',
  `distributeTime` bigint(20) NOT NULL DEFAULT '1000' COMMENT '分发时间默认：1000',
  `distributeStatus` int(1) NOT NULL DEFAULT '22' COMMENT '分发状态默认：已成功',
...
-- 多支付系统  系统日志
-- ----------------------------
CREATE TABLE IF NOT EXISTS `net_pay_log` (
