...
public class NetPayIdServiceImpl implements IdService {
	...
	public synchronized String createId()
	{
		...
		try
		{
			...
			sql.append("SELECT `id`, `date_time` FROM net_pay_id").append(" WHERE `date_time`='").append(d).append("'");
			...
			else
			{
				...
				sql.append("INSERT INTO net_pay_id").append("(`id`, `date_time`) VALUES ('1', '").append(d).append("')");
			...
			// 更新订单号
			sql = new StringBuffer();
			sql.append("UPDATE net_pay_id ").append(" SET id=id+1 WHERE ").append(" `date_time`='").append(id.getDateTime()).append("'  LIMIT 1");
	...
	private String _createId(Long id)
	{
		...
		Date date = new Date();
		return new StringBuffer("NPGW").append(new SimpleDateFormat("yyyy").format(date)).append(new SimpleDateFormat("MM").format(date)).append(new SimpleDateFormat("dd").format(date)).append(zero).toString();
	}
	
	public static void main(String[] args)
	{
		NetPayIdServiceImpl i = new NetPayIdServiceImpl();
