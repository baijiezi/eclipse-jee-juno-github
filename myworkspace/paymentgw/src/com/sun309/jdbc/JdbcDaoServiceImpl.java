...
public class JdbcDaoServiceImpl implements JdbcDaoService
{
	private Logger log = Logger.getLogger(JdbcDaoServiceImpl.class);
	...
	public List<Map<String, Object>> select(String sql, Connection conn)
	{
		...
		try
		{
			...
			if(Constants.SHOW_SQL) log.debug("Executing Select c sql : " + sql);
	...
	public long insert(String sql, Connection conn)
	{
		...
		{
			stmt = conn.prepareStatement(sql);
			if(Constants.SHOW_SQL) log.debug("Executing insert c sql : " + sql);
			stmt.executeUpdate();
	...
	public boolean update(String sql, Connection conn)
	{
		...
		{
			stmt = conn.prepareStatement(sql);
			if(Constants.SHOW_SQL) log.debug("Executing update sql : " + sql);
			stmt.executeUpdate();
			result = true;		
		}
		catch (Exception e)
	...
	public long insert(String sql)
	{
		...
		try
		{
			...
			stmt = conn.prepareStatement(sql);
			if(Constants.SHOW_SQL) log.debug("Executing insert sql : " + sql);
			stmt.executeUpdate();
			rs = stmt.getGeneratedKeys();
			while (rs.next())
			{
	...
	public int delete(String sql)
	{
		...
		try
		{
			...
			conn = service.getConn();
			stmt = conn.prepareStatement(sql);
			if(Constants.SHOW_SQL) log.debug("Executing delete sql : " + sql);
			result = stmt.executeUpdate();
			// 提交事务
			commit(conn);
		}
	...
	public boolean batchExecute(ArrayList<String> sql)
	{
		...
		try
		{
			...
			for (String _sql : sql)
			{
				...
				if(Constants.SHOW_SQL) log.debug("Executing batchExecute sql : " + _sql);
				stmt.executeUpdate();
			}
			commit(conn);
			result = true;
	...
	public boolean batchExecute(ArrayList<String> sql, Connection conn)
	{
		...
		try
		{
			...
			{
				stmt = conn.prepareStatement(_sql);
				if(Constants.SHOW_SQL) log.debug("Executing batchExecute c sql : " + _sql);
				stmt.executeUpdate();
			}
			result = true;
			sql=null;
	...
	public List<Map<String, Object>> select(String sql)
	{
		...
		try
		{
			...
			if(Constants.SHOW_SQL) log.debug("Executing select sql : " + sql);
